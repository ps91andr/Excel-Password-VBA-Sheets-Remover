![GitHub release](https://img.shields.io/github/v/release/ps91andr/Excel-Password-VBA-Sheets-Remover?style=for-the-badge&logo=github)
![GitHub downloads](https://img.shields.io/github/downloads/ps91andr/Excel-Password-VBA-Sheets-Remover/total?style=for-the-badge&logo=github)
![GitHub stars](https://img.shields.io/github/stars/ps91andr/Excel-Password-VBA-Sheets-Remover?style=for-the-badge&logo=github)
![GitHub forks](https://img.shields.io/github/forks/ps91andr/Excel-Password-VBA-Sheets-Remover?style=for-the-badge&logo=github)
![GitHub license](https://img.shields.io/github/license/ps91andr/Excel-Password-VBA-Sheets-Remover?style=for-the-badge)
![Platform](https://img.shields.io/badge/Platform-Windows-blue?style=for-the-badge&logo=windows)
![GitHub watchers](https://img.shields.io/github/watchers/ps91andr/Excel-Password-VBA-Sheets-Remover?style=for-the-badge&logo=github)

# Excel Password VBA Sheets Remover

Tool to remove Excel sheet and VBA passwords.

# 🔓 Excel-Password-VBA-Sheets-Remover

<div align="center">
  
  <h1>🔓 Excel Password Remover</h1>
  <h3>مزيل حماية ملفات إكسل | Excel Protection Remover</h3>
  <p><em>أداة سطح مكتب لإزالة جميع أنواع الحماية من ملفات Excel دفعة واحدة</em></p>
  
  <p>
    <img src="https://img.shields.io/badge/version-1.0.0-blue.svg" alt="Version 1.0.0">
    <img src="https://img.shields.io/badge/python-3.8+-green.svg" alt="Python 3.8+">
    <img src="https://img.shields.io/badge/PyQt-6.0-orange.svg" alt="PyQt6">
    <img src="https://img.shields.io/badge/license-MIT-yellow.svg" alt="MIT License">
    <img src="https://img.shields.io/badge/platform-windows%20%7C%20linux%20%7C%20macos-lightgrey" alt="Platforms">
  </p>
  
  <!-- روابط التنقل السريع -->
  <h3>
    <a href="#english_section">English</a> | <a href="#arabic_section">العربية</a>
  </h3>

  <p>
    <b>قائمة التنقل (العربية):</b><br>
    <a href="#features">المميزات</a> •
    <a href="#download">التحميل</a> •
    <a href="#usage">الاستخدام</a> •
    <a href="#formats">الصيغ</a> •
    <a href="#comparison">مقارنة</a> •
    <a href="#disclaimer">إخلاء المسؤولية</a> •
    <a href="#license">الترخيص</a>
  </p>

  <p>
    <b>Quick Navigation (English):</b><br>
    <a href="#Efeatures">Features</a> •
    <a href="#Edownload">Download</a> •
    <a href="#Eusage">Usage</a> •
    <a href="#Eformats">Formats</a> •
    <a href="#Edisclaimer">Disclaimer</a> •
    <a href="#Econtact">Contact</a>
  </p>
  
</div>

---

<div id="arabic_section"></div>

## 🎧 العربية

<div dir="rtl">

### نظرة عامة

**Excel Password Remover** هو أداة سطح مكتب ذكية ومفتوحة المصدر، مصممة لإزالة جميع أنواع الحماية من ملفات Excel بسهولة وأمان. يعمل التطبيق **محلياً بالكامل** على جهازك دون الحاجة للاتصال بالإنترنت، مع دعم كامل للغة العربية.

---

<div id="features"></div>

### ✨ المميزات

| الميزة | الوصف |
|--------|-------|
| 🔓 **إزالة حماية الصفحات** | إزالة حماية كلمة المرور من جميع أوراق العمل دفعة واحدة |
| 🔐 **إزالة حماية VBA** | فك حماية مشروع VBA وإزالة كلمة المرور |
| 📁 **معالجة مجمعة** | دعم إضافة ومعالجة عدة ملفات في وقت واحد |
| 🎯 **دعم جميع الصيغ** | xlsx, xlsm, xlsb, xlam, xls, xla, xltm, xltx |
| 💾 **خصوصية تامة** | معالجة محلية 100% - لا رفع لأي ملفات للإنترنت |
| 🖱️ **سحب وإفلات** | واجهة سهلة مع دعم السحب والإفلات |
| 🌓 **واجهة عصرية** | تصميم أنيق مع بطاقات ملونة لحالة كل ملف |
| 🌐 **ثنائي اللغة** | واجهة عربية وإنجليزية كاملة |
| ⚡ **معالجة متزامنة** | معالجة الملفات في خلفية التطبيق دون تجميد الواجهة |

---

### 🖼️ لقطات شاشة
![لقطة شاشة للواجهة الرئيسية](https://github.com/ps91andr/Excel-Password-VBA-Sheets-Remover/blob/main/Screenshot%202026-02-22%20195730.png)

---

<div id="download"></div>

### 📦 التحميل المباشر

#### نسخ جاهزة للتشغيل (Windows)

| الملف | الوصف |
|-------|-------|
| [Excel_Password_Remover_AR.exe](https://github.com/ps91andr/Excel-Password-VBA-Sheets-Remover/releases/tag/Excel_Password_Remover_AR_EN) | النسخة العربية - لا يحتاج تثبيت |
| [Excel_Password_Remover_AR.exe](https://www.up-4ever.net/6mgk38ds3lbt) | النسخة العربية - تحميل مباشر - لا يحتاج تثبيت |
| [Excel_Password_Remover_EN.exe](https://github.com/ps91andr/Excel-Password-VBA-Sheets-Remover/releases/tag/Excel_Password_Remover_AR_EN) | النسخة الإنجليزية - لا يحتاج تثبيت |
| [Excel_Password_Remover_EN.exe](https://www.up-4ever.net/4jli1bvht6r9) | النسخة الإنجليزية - تحميل مباشر - لا يحتاج تثبيت |

[![Direct Download](https://img.shields.io/badge/Direct-Download-blue?style=for-the-badge)](PUT_YOUR_DOWNLOAD_LINK_HERE)
[![Direct Download](https://img.shields.io/badge/Direct-Download-blue?style=for-the-badge)](https://example.com/file.zip)


| الملف | التحميل |
|------|------|
| النسخة العربية (GitHub) - لا يحتاج تثبيت  | [![Download](https://img.shields.io/badge/Download-AR%20Version-blue?style=for-the-badge&logo=download)](https://github.com/ps91andr/Excel-Password-VBA-Sheets-Remover/releases/tag/Excel_Password_Remover_AR_EN) |
| النسخة العربية (Download Direct) - لا يحتاج تثبيت  | [![Direct Download](https://img.shields.io/badge/Direct%20Download-AR-green?style=for-the-badge&logo=download)](https://www.up-4ever.net/6mgk38ds3lbt) |
| النسخة الإنجليزية (GitHub) - لا يحتاج تثبيت  | [![Download](https://img.shields.io/badge/Download-EN%20Version-blue?style=for-the-badge&logo=download)](https://github.com/ps91andr/Excel-Password-VBA-Sheets-Remover/releases/tag/Excel_Password_Remover_AR_EN) |
| النسخة الإنجليزية (Download Direct) - لا يحتاج تثبيت | [![Direct Download](https://img.shields.io/badge/Direct%20Download-EN-green?style=for-the-badge&logo=download)](https://www.up-4ever.net/4jli1bvht6r9) |



## 📥 Download

| Version | GitHub Release | Direct Download |
|--------|---------------|----------------|
| 🇸🇦 Arabic Version | [![GitHub Download](https://img.shields.io/badge/GitHub-Download-blue?style=for-the-badge&logo=github)](https://github.com/ps91andr/Excel-Password-VBA-Sheets-Remover/releases/tag/Excel_Password_Remover_AR_EN) | [![Direct Download](https://img.shields.io/badge/Direct-Download-green?style=for-the-badge&logo=download)](https://www.up-4ever.net/6mgk38ds3lbt) |
| 🇬🇧 English Version | [![GitHub Download](https://img.shields.io/badge/GitHub-Download-blue?style=for-the-badge&logo=github)](https://github.com/ps91andr/Excel-Password-VBA-Sheets-Remover/releases/tag/Excel_Password_Remover_AR_EN) | [![Direct Download](https://img.shields.io/badge/Direct-Download-green?style=for-the-badge&logo=download)](https://www.up-4ever.net/4jli1bvht6r9) |

> 💡 **ملاحظة:** فقط حمل وشغل مباشرة!

---

<div id="usage"></div>

### 🚀 طريقة الاستخدام

1. **التحضير**: احفظ نسخة احتياطية من ملفاتك المهمة
2. **التشغيل**: افتح التطبيق (اختر العربية أو الإنجليزية)
3. **الموافقة**: اقرأ إخلاء المسؤولية واضغط على "موافق"
4. **إضافة الملفات**:
   - اسحب وأفلت الملفات في المنطقة المخصصة
   - أو اضغط على زر "إضافة ملفات"
5. **الانتظار**: سيعالج التطبيق الملفات تلقائياً في الخلفية
6. **الحفظ**:
   - اضغط على أي ملف لحفظه منفرداً
   - أو استخدم زر "حفظ الكل" لحفظ جميع الملفات الناجحة

#### ⚠️ تنبيه مهم بعد إزالة حماية VBA

1. افتح الملف المعالج في Excel
2. ادخل إلى محرر VBA (Alt + F11)
3. ستظهر رسالة خطأ "Invalid Key" - اضغط OK
4. احفظ الملف (Ctrl + S) ثم أغلقه
5. أعد فتح الملف - ستجد محرر VBA مفتوحاً بدون كلمة مرور

---

<div id="formats"></div>

### 🎛️ الصيغ المدعومة

| الصيغة | الامتداد | إزالة حماية الصفحات | إزالة حماية VBA |
|--------|----------|---------------------|-----------------|
| Excel Workbook | `.xlsx` | ✅ | ❌ |
| Excel Macro-Enabled | `.xlsm` | ✅ | ✅ |
| Excel Binary | `.xlsb` | ✅ | ✅ |
| Excel Add-In | `.xlam` | ✅ | ✅ |
| Excel 97-2003 | `.xls` | ❌ | ✅ |
| Excel 97-2003 Add-In | `.xla` | ❌ | ✅ |
| Excel Template | `.xltx` | ✅ | ❌ |
| Macro-Enabled Template | `.xltm` | ✅ | ✅ |

---

<div id="disclaimer"></div>

### ⚖️ إخلاء المسؤولية

<div dir="rtl" style="background-color: #331f00; padding: 15px; border-radius: 5px; border-right: 5px solid #ffaa00;">

**🔴 مهم جداً - يرجى القراءة بعناية**

هذه الأداة مخصصة **للاستخدام القانوني فقط** على:
- الملفات التي تملكها أنت شخصياً
- الملفات التي لديك إذن صريح بتعديلها

باستخدام هذه الأداة، فإنك تقر وتوافق على أنك:

- تتحمل المسؤولية الكاملة عن استخدامها
- لن تستخدمها على ملفات لا تملكها أو بدون ترخيص
- تفهم أن المطور غير مسؤول عن أي استخدام غير قانوني

> **⚠️ براءة ذمة:** نتبرأ إلى الله من أي استخدام غير مشروع لهذه الأداة. من استخدمها على ملفات لا يملكها أو بدون ترخيص، فإثمه على نفسه، ونحن منها براء في الدنيا والآخرة.

</div>

---

<div id="comparison"></div>

### 📊 مقارنة مع أدوات أخرى

| الميزة | Excel Password Remover | أدوات تجارية | أدوات أونلاين |
|--------|------------------------|---------------|---------------|
| مجاني 100% | ✅ | ❌ | ✅/❌ |
| إزالة حماية VBA | ✅ | ✅ | ❌ |
| معالجة مجمعة | ✅ | ❌ | ❌ |
| خصوصية تامة | ✅ | ✅ | ❌ |
| دعم عربي | ✅ | ❌ | ❌ |
| مفتوح المصدر | ✅ | ❌ | ❌ |
| بدون تثبيت | ✅ | ❌ | ✅ |

---

<div id="contribution"></div>

### 🤝 المساهمة

نرحب بمساهماتكم! يمكنك المساهمة بعدة طرق:

- **الإبلاغ عن مشكلة**: افتح Issue إذا وجدت خطأ
- **اقتراح ميزة**: شاركنا أفكارك لتحسين الأداة
- **تحسين الترجمة**: ساعد في تحسين الترجمات العربية أو الإنجليزية
- **تطوير الكود**: أرسل Pull Request بتحسيناتك
- **مشاركة الأداة**: شارك المشروع مع من قد يستفيد

---

<div id="license"></div>

### 📄 الترخيص

هذا المشروع مرخص تحت **MIT License** - انظر ملف [LICENSE](LICENSE) للتفاصيل.

---

<div id="contact"></div>

### 📬 التواصل

- **البريد الإلكتروني**: [ps91andr@gmail.com](mailto:ps91andr@gmail.com)
- **GitHub**: [@ps91andr](https://github.com/ps91andr)
- **الموقع**: [https://github.com/ps91andr](https://github.com/ps91andr)

---

### ⭐ الدعم

إذا أعجبك المشروع، لا تنسى وضع نجمة ⭐ على GitHub! 👍

---

</div>

---

<div id="english_section"></div>

## English

<div dir="ltr">

### Overview

**Excel Password Remover** is an open-source desktop tool designed to remove all types of protection from Excel files easily and securely. The application works **100% locally** on your device without needing an internet connection, with full English interface support.

---

<div id="Efeatures"></div>

### ✨ Features

| Feature | Description |
|--------|-------------|
| 🔓 **Worksheet Protection** | Remove password protection from all worksheets at once |
| 🔐 **VBA Protection** | Unlock VBA project and remove password |
| 📁 **Batch Processing** | Add and process multiple files simultaneously |
| 🎯 **All Formats Support** | xlsx, xlsm, xlsb, xlam, xls, xla, xltm, xltx |
| 💾 **Full Privacy** | 100% local processing - no files uploaded |
| 🖱️ **Drag & Drop** | Easy-to-use interface with drag & drop support |
| 🌓 **Modern UI** | Clean design with colored cards for each file status |
| 🌐 **Bilingual** | Full Arabic and English interface |
| ⚡ **Async Processing** | Files processed in background without UI freezing |

---

### 🖼️ Screenshots
![Screenshots](https://github.com/ps91andr/Excel-Password-VBA-Sheets-Remover/blob/main/Screenshot%202026-02-22%20195220.png)

---

<div id="Edownload"></div>

### 📦 Direct Download

#### Ready-to-run versions (Windows)

| File | Description |
|------|-------------|
| [Excel_Password_Remover_AR.exe](https://github.com/ps91andr/Excel-Password-VBA-Sheets-Remover/releases/tag/Excel_Password_Remover_AR_EN) | Arabic version - no installation needed |
| [Excel_Password_Remover_AR.exe](https://www.up-4ever.net/6mgk38ds3lbt) | Arabic version - Direct download - no installation needed |
| [Excel_Password_Remover_EN.exe](https://github.com/ps91andr/Excel-Password-VBA-Sheets-Remover/releases/tag/Excel_Password_Remover_AR_EN) | English version - no installation needed |
| [Excel_Password_Remover_EN.exe](https://www.up-4ever.net/4jli1bvht6r9) | English version - Direct download - no installation needed |
---

<div id="Eusage"></div>

### 🚀 How to Use

1. **Preparation**: Backup your important files first
2. **Launch**: Open the application (choose Arabic or English)
3. **Accept**: Read the disclaimer and check "I agree"
4. **Add files**:
   - Drag and drop files into the drop area
   - Or click "Add Excel files" button
5. **Wait**: Application processes files automatically in background
6. **Save**:
   - Click any file to save it individually
   - Or use "Save all files" button

#### ⚠️ Important Note After VBA Protection Removal

1. Open the processed file in Excel
2. Enter VBA editor (Alt + F11)
3. "Invalid Key" error message appears - click OK
4. Save the file (Ctrl + S) and close it
5. Reopen the file - VBA editor will open without password

---

<div id="Eformats"></div>

### 🎛️ Supported Formats

| Format | Extension | Worksheet Protection | VBA Protection |
|--------|----------|---------------------|----------------|
| Excel Workbook | `.xlsx` | ✅ | ❌ |
| Excel Macro-Enabled | `.xlsm` | ✅ | ✅ |
| Excel Binary | `.xlsb` | ✅ | ✅ |
| Excel Add-In | `.xlam` | ✅ | ✅ |
| Excel 97-2003 | `.xls` | ❌ | ✅ |
| Excel 97-2003 Add-In | `.xla` | ❌ | ✅ |
| Excel Template | `.xltx` | ✅ | ❌ |
| Macro-Enabled Template | `.xltm` | ✅ | ✅ |

---

<div id="Edisclaimer"></div>

### ⚖️ Disclaimer

**🔴 Important - Read Carefully**

This tool is intended **for legal use only** on:
- Files that you personally own
- Files you have explicit permission to modify

By using this tool, you acknowledge and agree that you:
- Assume full responsibility for its use
- Will not use it on files you don't own or without authorization
- Understand that the developer is not responsible for any illegal use

---

<div id="Econtact"></div>

### 📬 Contact

- **Email**: [ps91andr@gmail.com](mailto:ps91andr@gmail.com)
- **GitHub**: [@ps91andr](https://github.com/ps91andr)
- **WEB**: [https://github.com/ps91andr](https://github.com/ps91andr)

---

<div align="center">
  
  ---
  
  **صنع بحب للمجتمع العربي والعالمي** ❤️  
  **Made with love for the community**
  
  <sub>© 2026 Excel Password Remover. All rights reserved.</sub>
  
</div>

</div>
